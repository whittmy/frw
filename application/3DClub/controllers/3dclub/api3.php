...
// http://www.nybgjd.com/3dclub/api3/
...
//		return;
	...
	function getClass(){
		...
		$sql = 'select t_id,t_name,t_pic,t_content,t_ext_squarelogo,t_ext_v3squarelogo,t_ext_transverse from vr_vod_topic order by t_sort';	
		$query = $this->db->query($sql);	
		foreach($query->result() as $row){
			$cif = array();
			$cif['id'] = $row->t_id.'';
			$cif['name'] = $row->t_name;
			$cif['logo'] = $host.$row->t_pic;
			$cif['squarelogo'] = $host.$row->t_ext_squarelogo;
			$cif['v3squarelogo'] = $host.$row->t_ext_v3squarelogo;
	...
	function getMovieTop(){
		...
		$data1 = $this->mp_cache->get('movietop');
		if($data1 === false){
			$this->load->database('vr');
			...
			$this->mp_cache->write($data1, 'movietop', 24*3600);
		}
	...
	function getGameList(){
		...
		foreach($query->result() as $row){
			...
			$cif['size'] = $row->d_size;
			$cif['images'] = $host.$row->d_pic;
			$cif['file'] = 'http://www.nybgjd.com/srv2/get/img/vrgame/'.urlencode($row->d_downurl);
	...
	function getGameDetail(){
		...
		foreach($query->result() as $row){
			...
			$cif['file'] =  'http://www.nybgjd.com/srv2/get/img/vrgame/'.urlencode($row->d_file);
	...
	function getGameTop(){
		...
		$data1 = $this->mp_cache->get('gametop'); 
		if($data1 === false){
			$ret = array();
			...
			$data1 = json_encode($ret);
			$this->mp_cache->write($data1, 'gametop', 24*3600);
		}
	...
	// quality: 0:smooth; 1:org
	function getDuMvUrl($usr=null, $fileId=null, $quality=1, $bcopy=1, $optype=null){
