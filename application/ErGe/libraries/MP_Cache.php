...
/**
*	使用说明： application/libraries
	config.php
		$config['mp_cache_dir'] = 'application/Moon_Market/application/mp_cache/';	//可写,
		$config['mp_cache_default_expires'] = 10;	// seconds
	
	usage:
	  1. 
		$this->load->library('MP_Cache'); 
		$data1 = $this->mp_cache->get('filename');	//可以加子目录 get('sub/filename')
		if ($data1 === false){
			...
			...
			$data1 = xxxxx;
			$this->mp_cache->write($data1, 'filename');	//$this->mp_cache->write($data, 'example', 7200);  
		}
