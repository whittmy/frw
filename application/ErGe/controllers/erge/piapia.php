...
class PiaPia  extends CI_Controller {
...
//		$stream = 'stream_id=mp4';
	...
	//这里的分类可以所有的分类、包括水平的
	function _getResListData($cids, $pgId, $pgsize=15){
		$cids = trim($cids, ',');
        if(strlen($cids) < 1)
            return null;
		$this->load->library('MP_Cache');
	...
	function _getHListArr($topCata, $pgidx=1, $pgsize=20){
    ...
    */
	...
	function getresList($bgencache=0){
		...
		if($data1 === false || $bgencache==1){
           $data1 = $this->_genResListCache($fid, $pgId, $pgsize,$cacheName);
		}
		exit($data1);
	}		

    function _genResListCache($fid, $pgId, $pgsize,$cacheName){
        //echo 'cache.<br>';
        require(APPPATH.'/controllers/erge/PUBLIC_CFG/cate_info_cache.php');
        ...
        if(isset($TOPIC_INFO_CACHE[$fid])){
            ...
            if($pgId == 1 && $fid!=-1){
                $headerList = $this->_getHListArr($fid);
            }
        }	
        //$this->db->close();
