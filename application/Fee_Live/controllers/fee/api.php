...
class Api extends CI_Controller {
	...
	*/
	...
	function _getUsrInfo($uid=null, $wmac=null, $lmac=null){
		...
		$this->load->database();
		$sql = 'SELECT wmac1,wmac2,lmac1,lmac2,(exp_date-TIMESTAMPDIFF(DAY, tm_firstlogin, now())) remaindays, exp_date, bpause, utype FROM __feelive_user_info where uid='.$uid;		
			...
			if($remaindays==null ){
				...
				if(($wmac1!=0 || $wmac2!=0) && ($lmac1!=0 || $lmac2!=0)){
					...
					if($wmac1==$wifiMac1 && $wmac2==$wifiMac2
						&& $lmac1==$lanMac1 && $lmac2==$lanMac2){
						//机器信息匹配成功
						
						//激活 tm_firstlogin
						$sql = 'update __feelive_user_info set tm_firstlogin=now() where uid='.$uid;
						$this->db->query($sql);
					...
					// 不存在机器信息（MAC地址等信息）
					$sql = 'update __feelive_user_info set wmac1='.$wifiMac1.
										', wmac2='.$wifiMac2.
										', lmac1='.$lanMac1.
										', lmac2='.$lanMac2.
	...
	function gen_guid($cnt, $type='1', $expr=1, $bsave=0){
		$this->load->database();
		$sql = 'SELECT count(id) as cnt FROM `__feelive_user_info`';
		$query = $this->db->query($sql);
		...
		$sql_pre = 'insert into __feelive_user_info(uid, exp_date, utype) values ';
		$sql_f = null;
		foreach($ids as $id){
